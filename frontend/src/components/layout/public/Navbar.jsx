...
const Navbar = ({ sidebarOpen, setSidebarOpen }) => {
  const { user: auth, isLoadingUser, logout } = useAuth();
  const { pathname } = useLocation();
  const navigate = useNavigate();
  const [theme] = useState(localStorage.getItem('theme') ? localStorage.getItem('theme') : 'light');
  const handleSignOut = () => {
          ...
          {isLoadingUser ? (
            ...
            <div className="dropdown dropdown-end">
              <div className="indicator">
                ...
                <label tabIndex={0} className="btn btn-circle btn-ghost btn-sm m-1 font-notosanslao">
              ...
              >
                <Link className={`btn btn-ghost btn-sm relative whitespace-nowrap`} to={`/profile`}>
                  <FaRegIdCard size={17} />
                ...
                <button onClick={handleSignOut} className="btn btn-ghost btn-sm whitespace-nowrap">

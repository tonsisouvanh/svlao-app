...
export const createAnnouncement = createAsyncThunk('announcements/addAnnouncement', async (inputData, thunkAPI) => {
  try {
    const config = {
      headers: {
    ...
    const { data } = await apiRequestPrivate.post('/announcements', inputData, config);
    return { data };
  } catch (error) {
    const message =
...
export const updateAnnouncement = createAsyncThunk('announcements/updateAnnouncement', async (inputData, thunkAPI) => {
  try {
    const config = {
      headers: {
        'Content-Type': 'application/json',
...
export const removeAnnouncement = createAsyncThunk(
  ...
  async (announcementId, thunkAPI) => {
    try {
      ...
      const res = await apiRequestPrivate.delete(`/announcements/${announcementId}`, config);
...
export const getAnnouncementById = createAsyncThunk(
  ...
  async (announcementId, thunkAPI) => {
    try {
      ...
      const { data } = await apiRequest.get(`/announcements/${announcementId}`, config);
      return data;
    } catch (error) {
      const message =
        (error.response && error.response.data && error.response.data.message) || error.message || error.toString();
...
export const countViews = createAsyncThunk('announcements/countVies', async (announcementId, thunkAPI) => {
  try {
    const config = {
      headers: {
        'Content-Type': 'application/json',
    ...
    const { data } = await apiRequest.post(`/announcements/${announcementId}`, config);
    return data;
  } catch (error) {
    const message =
      (error.response && error.response.data && error.response.data.message) || error.message || error.toString();

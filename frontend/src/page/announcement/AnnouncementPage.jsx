...
const AnnouncementPage = () => {
  ...
  } = useGetAllAnnouncements(pageNumber, keyword);
  // const { announcements, page, pages } = data;

  useEffect(() => {
    refetchAnnouncement();
  }, [pageNumber, refetchAnnouncement]);

  if (isAnnouncementLoading) return <Spinner />;
  return (
    <>
      <div>
                    ...
                    {data.announcements.map((announcement) => (
                      <div key={announcement._id} className="mb-6 flex flex-wrap">
                        <div className="mb-6 ml-auto w-full shrink-0 grow-0 basis-auto px-3 md:mb-0 md:w-3/12">
                          ...
                          >
                            <img
                              ...
                              onError={(error) => replaceImage(error, images.defaultImage)}
                            />
                            <Link
                              // onClick={() => dispatch(countViews(announcement._id))}
                        ...
                        <div className="mb-6 mr-auto w-full shrink-0 grow-0 basis-auto px-3 md:mb-0 md:w-9/12 xl:w-7/12">
                          <h5 className="mb-3 text-lg font-bold">{announcement.title}</h5>
